...
class FuzzyLogicController_RE:
    ...
    def __init__(self):
        ...
        self.low = [0, 0.03, 0.06]
        self.med = [0.06, 0.09, 0.12]
        self.high = [0.12, 0.15, 0.18]
...
# Generates a movement command based on both fuzzy RE and OA controllers sensor readings
def movement():
    FLS = regions_['fleft']
    ...
    mynode_.get_logger().info(f"FLS: {FLS:.2f}, FMS: {FMS:.2f}, FRS: {FRS:.2f}, RBS: {RBS:.2f}")
    
    '''Creating instances of both fuzzy controllers
    So that the rule base/ defuzzification function can be called in the following line of code'''
    re = FuzzyLogicController_RE()
    oa = FuzzyLogicController_OA()

    '''Where if the minimum distance from the front sensors is less than 0.6... implement obstacle avoidance
    It should be noted that the distance value was tuned to achieve optimised results
    Hence, 0.6 metres was not a guess'''
    if min(FLS, FMS, FRS) <= 0.6:
        linear_speed, angular_speed = oa.rule_base_defuzzy(FLS, FMS, FRS)
    # Else... Keep following the right-edge
    else:
        linear_speed, angular_speed = re.rule_base_defuzzy(RFS, RBS)
